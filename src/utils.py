...
def load_config(config_path: str = PROJECT_ROOT / "configs" / "config.yaml") -> dict:
...
def save_checkpoint(run_dir, epoch, model, optimizer,
    ...
    """Save a periodic checkpoint so training can be resumed."""
...
def save_best_model(run_dir, epoch, model, val_acc):
    """Overwrite best_model.pt whenever validation accuracy improves."""
    ...
    print(f"  [Checkpoint] New best model (val_acc={val_acc:.4f}) -> {path}")


def load_checkpoint(path, model, optimizer=None):
    """
    Load a checkpoint back into model (and optionally optimizer).
    Returns the checkpoint dict so callers can restore epoch/metrics.
    """
